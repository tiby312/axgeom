...
macro_rules! impl_float {
    ( $x:ty ) => {
        impl Ray<$x> {
            ...
            pub fn cast_to_circle(&self, center: Vec2<$x>, radius: $x) -> CastResult<$x> {
                ...
                match roots::find_roots_quadratic(a, b, c) {
                    roots::Roots::No(_) => CastResult::NoHit,
                    roots::Roots::One([a]) => {
                        if a < zz {
                            CastResult::NoHit
                        } else {
            ...
            pub fn cast_to_aaline<A: Axis>(&self, a: A, line: $x) -> CastResult<$x> {
                let ray = self;
            ...
            pub fn cast_to_rect(&self, rect: &Rect<$x>) -> CastResult<$x> {
                if rect.contains_point(self.point) {
                    return CastResult::Hit(0.0);
                }
                /*
                ...
                */
                ...
                let x = if self.dir.x >= 0.0 { startx } else { endx };

                let y = if self.dir.y >= 0.0 { starty } else { endy };
